...
class audio_route:
    ...
    def on_call_start(self):
        if self.aplay_sco is None:
            self.aplay_sco = Popen([self.bluealsa_aplay, "--profile-sco"], stdout=PIPE, stderr=PIPE, shell=True)
        ...
        self.arec_mic = Popen([self.arecord, "-D plughw:1 -f S16_LE -c 1 -r 16000 mic.wav"], stdout=PIPE, shell=True)
        self.aplay_mic = Popen([self.aplay, "-D bluealsa:SRV=org.bluealsa,DEV="+self.device_id+",PROFILE=sco mic.wav"], stdout=PIPE, stdin=self.arec_mic.stdout, shell=True)